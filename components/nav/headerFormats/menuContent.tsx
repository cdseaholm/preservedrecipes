...
export default function MenuContent({ userInfo, session, profile, firstName, setOpenSignOutModal, signOut, setSignInModal, signIn, setRegisterModal, handleZoomClick }: { userInfo: IUser, handleZoomClick: () => void; handleZoomClose: () => void; profile: React.ReactNode; firstName: string | null; setOpenSignOutModal: (open: boolean) => void; signOut: JSX.Element; setSignInModal: (open: boolean) => void, setRegisterModal: (open: boolean) => void, session: Session | null, signIn: JSX.Element | null }) {
    ...
    return session ? (
        ...
        <Menu.Dropdown style={{ border: '1px solid #716040', outlineOffset: '-2px' }}>
            ...
            <Menu.Item onClick={() => setRegisterModal(true)} pb={'sm'}>Register</Menu.Item>
        </Menu.Dropdown>
    );
};